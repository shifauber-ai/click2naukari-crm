...
import { format, startOfWeek, startOfMonth, isSameDay, isSameWeek, isSameMonth, subDays } from "date-fns";

type CarTab = "payment" | "leads" | "cities";

// Product codes that map to CAR
const CAR_PRODUCT_CODES = ["MAINC001", "CAR"];

export default function CarPage() {
  const [product, setProduct] = useState<Product | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<CarTab>("leads");

  useEffect(() => {
    (async () => {
      // Try multiple resolution strategies to find the CAR product
      // 1. Match by known codes
      for (const code of CAR_PRODUCT_CODES) {
      ...
      const { data: nameMatch } = await supabase
        ...
        .maybeSingle();
      if (nameMatch) {
        setProduct(nameMatch as Product);
        setLoading(false);
        return;
      }
      // 3. Match by name containing "car"
      ...
      if (containsMatch && containsMatch.length > 0) {
        setProduct(containsMatch[0] as Product);
        setLoading(false);
        return;
      }
      setLoading(false);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <div className="rounded-xl border border-border/60 bg-card p-8 text-center max-w-md">
          ...
          <h3 className="text-lg font-semibold mb-1">CAR Product Not Found</h3>
          <p className="text-sm text-muted-foreground mb-4">
            The CAR product does not exist in the database yet. Create it from the Products page
            with code <code className="bg-muted px-1.5 py-0.5 rounded text-xs">MAINC001</code> or
            <code className="bg-muted px-1.5 py-0.5 rounded text-xs ml-1">CAR</code>.
          </p>
  ...
  return (
    <div>
      <PageHeader
        title="CAR Dashboard"
        description="Payment, Leads, and City management for CAR product"
        icon={Wallet}
      />

      {/* Tab navigation */}
      <div className="mb-6 flex gap-1 rounded-xl border border-border/60 bg-card p-1">
        <TabButton active={activeTab === "leads"} onClick={() => setActiveTab("leads")} icon={Phone} label="Leads" />
        <TabButton active={activeTab === "payment"} onClick={() => setActiveTab("payment")} icon={Wallet} label="Payment" />
        <TabButton active={activeTab === "cities"} onClick={() => setActiveTab("cities")} icon={MapPin} label="Cities" />
      </div>

      {activeTab === "leads" && <CarLeadsSection product={product} />}
      {activeTab === "payment" && <CarPaymentSection product={product} />}
      {activeTab === "cities" && <CarCitiesSection product={product} />}
    </div>
  );
}
...
// ============ LEADS SECTION ============
function CarLeadsSection({ product }: { product: Product }) {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: emps }, { data: c }, { data: cq }] = await Promise.all([
        ...
        supabase.from("product_cities").select("*").eq("product_id", product.id).order("city_name"),
  ...
  const load = useCallback(async () => {
    if (!loaded) return;
    setLoading(true);
    let cq = supabase.from("leads").select("*", { count: "exact", head: true }).eq("product_id", product.id).eq("is_active", true);
    let q = supabase
      .from("leads")
    ...
    if (search) { cq = cq.or(`name.ilike.%${search}%,phone.ilike.%${search}%`); q = q.or(`name.ilike.%${search}%,phone.ilike.%${search}%`); }
  ...
  useEffect(() => { setSelectedIds(new Set()); }, [cityFilter, statusFilter, employeeFilter, dateFrom, dateTo, search, page]);

  const employeeMap = new Map(employees.map((e) => [e.id, e]));
  const productCallers = new Set(callerQueues.filter((cq) => cq.is_active).map((cq) => cq.employee_id));
  const eligibleCallers = employees.filter((e) => e.is_active && productCallers.has(e.id));
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Also update city if changed
      if (editCity !== (editLead.city || "")) {
        await supabase.from("leads").update({ city: editCity || null }).eq("id", editLead.id);
      }
  ...
  const handleAssignSave = async () => {
    ...
    const { error } = await supabase.rpc("admin_reassign_lead", {
      ...
      p_remarks: "Manual assignment from CAR section",
    });
    if (error) {
      toast({ title: error.message, variant: "destructive" });
    } else {
  ...
  const handleBulkDirectory = async () => {
    ...
    let successCount = 0, dupCount = 0, failCount = 0;
    for (let i = 0; i < entries.length; i += 25) {
      ...
      else { successCount += (data?.length || 0); dupCount += chunk.length - (data?.length || 0); }
    }
    if (failCount > 0) {
      toast({ title: `${successCount} saved, ${dupCount} duplicates, ${failCount} failed`, variant: "destructive" });
    } else {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        ...
        <Select value={cityFilter} onValueChange={(v) => { setCityFilter(v); setPage(0); }}>
          ...
          <SelectContent>
            ...
            {cities.map((c) => <SelectItem key={c.id} value={c.city_name}>{c.city_name}{!c.is_active && " (inactive)"}</SelectItem>)}
          </SelectContent>
        </Select>
      ...
      {selectedIds.size > 0 && (
        <div className="mb-3 flex flex-col gap-3 rounded-xl border border-primary/20 bg-primary/5 p-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" onClick={() => setBulkAssignOpen(true)}><UserPlus className="mr-2 h-4 w-4" /> Assign</Button>
            <Button size="sm" variant="outline" onClick={() => setBulkDirOpen(true)}><BookMarked className="mr-2 h-4 w-4" /> Save to Directory</Button>
      ...
      {loading ? (
        ...
        <EmptyState icon={Phone} title="No CAR leads found" description="Adjust your filters or import leads for the CAR product." />
      ) : (
              ...
              <TableBody>
                {leads.map((lead) => (
                  <TableRow key={lead.id} className={selectedIds.has(lead.id) ? "bg-primary/5" : undefined}>
                    <TableCell><Checkbox checked={selectedIds.has(lead.id)} onCheckedChange={() => toggleSelect(lead.id)} aria-label={`Select ${lead.name}`} /></TableCell>
                    <TableCell className="font-medium">{lead.name}</TableCell>
                    <TableCell className="text-sm">{lead.phone}</TableCell>
                    <TableCell className="text-sm">{lead.city || "—"}</TableCell>
                    <TableCell><StatusBadge status={lead.status} /></TableCell>
                    <TableCell className="text-sm">{lead.current_caller ? employeeMap.get(lead.current_caller_id!)?.full_name || lead.current_caller?.full_name || "—" : "Unassigned"}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <TooltipProvider delayDuration={300}>
                          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(lead)}><Pencil className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent>Edit</TooltipContent></Tooltip>
                        </TooltipProvider>
                        <TooltipProvider delayDuration={300}>
                          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openAssign(lead)}><UserPlus className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent>Assign</TooltipContent></Tooltip>
                        </TooltipProvider>
                      </div>
                    </TableCell>
                  </TableRow>
      ...
      <Dialog open={!!editLead} onOpenChange={() => setEditLead(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Edit CAR Lead</DialogTitle></DialogHeader>
          <form onSubmit={handleEditSave} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={editSaving}>{editSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!assignLead} onOpenChange={() => setAssignLead(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Assign Lead</DialogTitle><DialogDescription>{assignLead?.name} ({assignLead?.phone})</DialogDescription></DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={assignCallerId} onValueChange={setAssignCallerId}>
                ...
                <SelectContent>{eligibleCallers.map((emp) => <SelectItem key={emp.id} value={emp.id}>{emp.full_name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setAssignLead(null)}>Cancel</Button>
              <Button onClick={handleAssignSave} disabled={assignSaving || !assignCallerId}>{assignSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Assign</Button>
            </DialogFooter>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={bulkAssignOpen} onOpenChange={() => setBulkAssignOpen(false)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Bulk Assign CAR Leads</DialogTitle><DialogDescription>{selectedIds.size} leads selected</DialogDescription></DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Assign To</Label>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleBulkAssign} disabled={bulkSaving || !bulkCallerId}>{bulkSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Assign</Button>
            </DialogFooter>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={bulkDirOpen} onOpenChange={() => setBulkDirOpen(false)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Save to Directory</DialogTitle><DialogDescription>Save {selectedIds.size} leads to Directory. Duplicates will be skipped.</DialogDescription></DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBulkDirOpen(false)}>Cancel</Button>
            <Button onClick={handleBulkDirectory} disabled={bulkDirSaving}>{bulkDirSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
// ============ PAYMENT SECTION ============
function CarPaymentSection({ product }: { product: Product }) {
  ...
  // Summary calculations from current page records
  const now = new Date();
  ...
  const todayCollection = completedRecords.filter((r) => r.payment_date && isSameDay(new Date(r.payment_date), now)).reduce((sum, r) => sum + Number(r.amount), 0);
  const weekCollection = completedRecords.filter((r) => r.payment_date && isSameWeek(new Date(r.payment_date), now, { weekStartsOn: 1 })).reduce((sum, r) => sum + Number(r.amount), 0);
  const monthCollection = completedRecords.filter((r) => r.payment_date && isSameMonth(new Date(r.payment_date), now)).reduce((sum, r) => sum + Number(r.amount), 0);
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setAddOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add Record</Button>
      </div>

      {/* Filters */}
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        <Input placeholder="Search candidate or txn ID..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} className="flex-1 min-w-[150px]" />
        <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(0); }}>
      ...
      {empAgg.size > 0 && (
        <div className="mb-4 rounded-xl border border-border/60 bg-card overflow-hidden">
          <div className="border-b border-border/60 px-4 py-2.5"><span className="text-sm font-semibold">Employee-wise Collection</span></div>
          <div className="overflow-x-auto scrollbar-thin">
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {Array.from(empAgg.entries()).map(([eid, agg]) => (
                  <TableRow key={eid}>
                    <TableCell className="font-medium">{eid === "none" ? "Unassigned" : employeeMap.get(eid)?.full_name || "—"}</TableCell>
                    <TableCell className="text-sm">{agg.count}</TableCell>
                    <TableCell className="font-medium">₹{agg.total.toLocaleString("en-IN")}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{agg.lastDate ? format(new Date(agg.lastDate), "dd MMM yyyy") : "—"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ...
          <div className="overflow-x-auto scrollbar-thin">
            <Table>
              ...
              <TableBody>
                {records.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="text-sm">{r.payment_date ? format(new Date(r.payment_date), "dd MMM yyyy") : "—"}</TableCell>
      ...
      <Dialog open={addOpen} onOpenChange={() => setAddOpen(false)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Add CAR Payment Record</DialogTitle><DialogDescription>Record a payment collection for CAR product</DialogDescription></DialogHeader>
...
function SummaryCard({ icon: Icon, label, value, variant }: { icon: any; label: string; value: string; variant?: "warning" | "error" }) {
  return (
    <div className={`rounded-xl border bg-card p-4 ${variant === "warning" ? "border-warning/30" : variant === "error" ? "border-destructive/30" : "border-border/60"}`}>
      <div className="flex items-center gap-2 text-xs text-muted-foreground mb-1">
        <Icon className="h-3 w-3" /> {label}
      </div>
      <p className={`text-lg font-bold ${variant === "warning" ? "text-warning" : variant === "error" ? "text-destructive" : ""}`}>{value}</p>
    </div>
  );
}

// ============ CITIES SECTION ============
function CarCitiesSection({ product }: { product: Product }) {
  const [cities, setCities] = useState<ProductCity[]>([]);
  const [loading, setLoading] = useState(true);
  const [addOpen, setAddOpen] = useState(false);
  ...
  const handleDelete = async () => {
    ...
    if (leadCount > 0) {
      toast({ title: `Cannot delete: ${leadCount} leads are associated with ${deleteCity.city_name}. Deactivate instead.`, variant: "destructive" });
      setDeleteCity(null);
      return;
    }
    const { error } = await supabase.from("product_cities").delete().eq("id", deleteCity.id);
    if (error) {
      toast({ title: error.message, variant: "destructive" });
    } else {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold">CAR Cities</h3>
          <p className="text-sm text-muted-foreground">Manage cities for CAR leads. Active cities appear in lead city dropdowns.</p>
        </div>
        <Button size="sm" onClick={() => setAddOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add City</Button>
      </div>

      {loading ? (
        <LoadingState />
      ) : cities.length === 0 ? (
        <EmptyState icon={MapPin} title="No cities configured" description="Add cities where CAR operates to organize leads by location." />
      ) : (
        <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
          <div className="overflow-x-auto scrollbar-thin">
            <Table>
              ...
              <TableBody>
                {cities.map((city) => (
                  <TableRow key={city.id} className={!city.is_active ? "opacity-60" : undefined}>
                    ...
                    <TableCell className="text-sm text-muted-foreground">{format(new Date(city.created_at), "dd MMM yyyy")}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{format(new Date(city.updated_at), "dd MMM yyyy")}</TableCell>
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(city)}><Pencil className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10" onClick={() => setDeleteCity(city)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
      ...
      <Dialog open={addOpen} onOpenChange={() => setAddOpen(false)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>Add CAR City</DialogTitle></DialogHeader>
          <form onSubmit={handleAdd} className="space-y-4">
            <div className="space-y-2"><Label>City Name</Label><Input value={newCityName} onChange={(e) => setNewCityName(e.target.value)} required placeholder="e.g. Mumbai" /></div>
            <div className="flex items-center gap-2">
              <Checkbox checked={newCityActive} onCheckedChange={(v) => setNewCityActive(!!v)} id="newCityActive" />
              <Label htmlFor="newCityActive">Active</Label>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setAddOpen(false)}>Cancel</Button>
              <Button type="submit" disabled={saving}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Add City</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editCity} onOpenChange={() => setEditCity(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <form onSubmit={handleEditSave} className="space-y-4">
            <div className="space-y-2"><Label>City Name</Label><Input value={editName} onChange={(e) => setEditName(e.target.value)} required /></div>
            <div className="flex items-center gap-2">
              <Checkbox checked={editActive} onCheckedChange={(v) => setEditActive(!!v)} id="editCityActive" />
      ...
      <Dialog open={!!deleteCity} onOpenChange={() => setDeleteCity(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDelete} disabled={!!deleteCity && (leadCounts[deleteCity.id] > 0)}>Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
