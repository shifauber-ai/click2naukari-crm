-- Fix: update_lead_platform_status should also sync leads.status
-- so that the All Leads table, Lead Details, and Status filter
-- all show the same status as the per-platform status.
-- Platform statuses that don't map directly to a LeadStatus are mapped:
--   FRESH -> NEW, EXISTING -> RINGING, OTHER_NUMBER -> OTHER_ISSUE,
--   PAYMENT_ISSUE -> OTHER_ISSUE, DONE -> ID_DONE
-- Direct matches (RINGING, ID_DONE, ID_BLOCK, DOC_ISSUE, VEHICLE_ISSUE,
...
BEGIN
  ...
  -- Map platform status to lead status and sync leads.status
  ...
  SELECT p_lead_id, v_lead_product_id, auth.uid(), l.status, v_mapped_status,
         p_platform_name || ' status: ' || p_status, 'ADMIN', auth.uid()
  FROM public.leads l
