-- Add p_remarks parameter to update_lead_platform_status RPC
-- so users can enter remarks when changing a lead's platform status.
-- The lead_status_history table already has a remarks column (text, NOT NULL DEFAULT '').
...
BEGIN
  ...
  -- Use user-provided remarks if non-empty, otherwise auto-generate
  v_remarks := COALESCE(NULLIF(trim(p_remarks), ''), p_platform_name || ' status: ' || p_status);
